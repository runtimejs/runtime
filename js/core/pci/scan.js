...
const pciAccessorFactory = ((addressPort, dataPort) => {
  ...
  const setPort = (bus, slot, func, offset) => {
    const addr = ((bus << 16) | (slot << 11) | (func << 8) |
      (offset & 0xfc) | 0x80000000) >>> 0;
    addressPort.write32(addr);
  };

  const readRaw32 = (bus, slot, func, offset) => {
    if (offset % sizeof.UINT32 !== 0) throw new Error('unaligned pci space 32 bit read');

    setPort(bus, slot, func, offset);
    return dataPort.read32();
  };

  const dwToFieldValue = (value, field) => ((value >>> (8 * sizeof.BYTE * field.shift)) & field.mask) >>> 0; // eslint-disable-line max-len
  ...
  class PciAccessor {
    constructor(address) {
      ...
      if (bus > 255) throw new Error('invalid bus value (expected 0-255)');
      if (slot > 31) throw new Error('invalid slot value (expected 0-31)');
      if (func > 7) throw new Error('invalid func value (expected 0-7)');

      const offsetCache = new Map();

      const writeRaw32 = (offset, value) => {
        if (offset % sizeof.UINT32 !== 0) throw new Error('unaligned pci space 32-bit write');

        setPort(bus, slot, func, offset);
        return dataPort.write32(value >>> 0);
      };

      const writeRaw16 = (offset, value) => {
        if (offset % sizeof.UINT16 !== 0) throw new Error('unaligned pci space 16-bit write');

        setPort(bus, slot, func, offset);
        return dataPort.write16((value & 0xffff) >>> 0);
      };

      const writeRaw8 = (offset, value) => {
        setPort(bus, slot, func, offset);
 ...
 */
const locateAcpiDevice = (dev) => {
  if (!dev.isDevice()) return null;
  ...
  if (!parentDev.isDevice()) return null;
  ...
  if (headerType !== 0x01 && headerType !== 0x02) return null;
...
const pciSpace = ((pciAccessorFactoryArg) => {
  const checkDevice = (bus, slot, func, fn) => {
    const addr = { bus, slot, func };
  ...
  const checkDeviceFunctions = (bus, slot, fn) => {
    let func = 0;
    const pciAccessor = pciAccessorFactoryArg.get({ bus, slot, func });
    ...
    for (func = 0; func < funcCount; ++func) checkDevice(bus, slot, func, fn);
  };
    ...
    eachDevice(fn) {
      ...
      for (let bus = 0; bus < 255; ++bus) {
        for (let slot = 0; slot < 32; ++slot) {
          if (!pciAccessorFactoryArg.exists(bus, slot, func)) continue;
...
const codeNameResolver = (() => {
  ...
  return {
    ...
    classCodeToName(code) {
      if (typeof classCodes[code] === 'undefined') return classCodes[0];
...
class PciDevice {
  constructor(address, pciAccessor) {
    ...
    const headerType = (header & 0x7f) >>> 0;
    if (headerType === 0x01 || headerType === 0x02) isBridge = true;
    ...
    this.attachAcpiDevice = (acpiDevice) => {
      if (that.acpiDevice !== null) return;
    ...
    this.getSecondaryBus = () => {
      if (!isBridge) throw new Error('device is not a bridge');
    ...
    this.setIRQVector = (vector) => {
      if (irqVector !== null) throw new Error('IRQ vector already set');
      irqVector = vector >>> 0;
    };
     ...
     */
    this.interruptPin = () => {
      if (isBridge) throw new Error('device is a bridge');
    ...
    this.getBAR = (index) => {
      ...
      if (indexValue > 5) throw new Error('invalid BAR register index (expected 0-5)');
      ...
      if (barType === null || obj === null) return null;
...
const pciManager = (() => {
  ...
  const addressHash = address => JSON.stringify([address.bus, address.slot, address.func]);
  ...
  return {
    ...
    findDevice(address) {
      const key = addressHash(address);
      if (!devicesMap.has(key)) return null;
...
acpiDevicesBuses.forEach((acpiDevice, bus) => {
  if (typeof acpiDevice === 'undefined') return;
...
pciManager.each((pciDevice) => {
  ...
  if (pciDevice.isBridge()) return;

  if (typeof busRouting[address.bus] === 'undefined') return; // No ACPI routing for this current device bridge
  ...
  if (devicePin === 0) return;

  routing.forEach((route) => {
    if (address.slot !== route.deviceId) return;
  ...
  if (deviceIRQ !== null) pciDevice.setIRQVector(deviceIRQ);
});
...
pciManager.each((pciDevice) => {
  ...
  if (driverData === null || typeof driverData.driver === 'undefined' || !driverData.enabled) return; // eslint-disable-line max-len
...
pciManager.each((pciDevice) => {
  ...
  let devicePin = 0;
  if (!pciDevice.isBridge()) devicePin = pciDevice.interruptPin();

  const pins = ['dont use', 'A', 'B', 'C', 'D'];

  const info = `${address.bus.toString(16)}: ${address.slot.toString(16)}.${address.func}
${pciDevice.vendorId().toString(16)}: ${pciDevice.deviceId().toString(16)}
${classData.className} IRQ: ${vector} PIN: ${pins[devicePin]}`;
  debug(info);
});

const listPciDevices = () => {
  const results = [];
