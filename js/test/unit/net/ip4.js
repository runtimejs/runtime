...
test('receive ip4 udp', (t) => {
  ...
  const udp = packetBuilder.createUDP(new Uint8Array([1, 2, 3, 4, 5]), { srcPort: 999, destPort: 65432 });
  const ip4 = packetBuilder.createEthernetIP4('udp', udp, { srcIP: '33.44.55.66' });
  intf.receive(ip4);
});

const ipFragmentsTest = (t, name, length, slices, order, norecv) => {
  t.test(name, (t2) => {
