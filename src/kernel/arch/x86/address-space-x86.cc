...
#include "address-space-x86.h"
#include <stdio.h>
#include <kernel/mem-manager.h>

namespace rt {

AddressSpaceX86::AddressSpaceX86(PageTableAllocator* table_allocator)
...
void AddressSpaceX86::Configure() {
...
void AddressSpaceX86::Install() {
  asm volatile("mov %0, %%cr3":: "b"(cr3_.Encode()));
}

class VirtualAddressX86 {
public:
  explicit VirtualAddressX86(void* address)
    : address_(reinterpret_cast<uintptr_t>(address)) {}
  explicit VirtualAddressX86(uintptr_t address)
    : address_(address) {}
...
void* AddressSpaceX86::VirtualToPhysical(void* virtaddr) {
  VirtualAddressX86 vaddr(virtaddr);
...
void AddressSpaceX86::MapPage(void* virtaddr, void* physaddr, bool invalidate, bool writethrough) {
  VirtualAddressX86 vaddr(virtaddr);
