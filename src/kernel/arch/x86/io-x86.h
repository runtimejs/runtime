...
public:
  inline static void Init() {
    port_ = 0x3F8;						  // COM1
    IoPortsX86::OutB(port_ + 1, 0x00);    // Disable all interrupts
    IoPortsX86::OutB(port_ + 3, 0x80);    // Enable DLAB (set baud rate divisor)
    IoPortsX86::OutB(port_ + 0, 0x03);    // Set divisor to 3 (lo byte) 38400 baud
    IoPortsX86::OutB(port_ + 1, 0x00);    //                  (hi byte)
    IoPortsX86::OutB(port_ + 3, 0x03);    // 8 bits, no parity, one stop bit
    IoPortsX86::OutB(port_ + 2, 0xC7);    // Enable FIFO, clear them, with 14-byte threshold
    IoPortsX86::OutB(port_ + 4, 0x0B);    // IRQs enabled, RTS/DSR set
  }
  ...
  inline static int is_transmit_empty() {
    return IoPortsX86::InB(port_ + 5) & 0x20;
  }

  inline static void write_serial(char a) {
    while (is_transmit_empty() == 0);
    IoPortsX86::OutB(port_, a);
  }
private:
  static int port_;
};
