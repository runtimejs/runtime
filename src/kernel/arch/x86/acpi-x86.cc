...
void AcpiX86::ParseRSDT(AcpiHeader* ptr) {
  RT_ASSERT(ptr);
...
void AcpiX86::ParseDT(AcpiHeader* ptr) {
  RT_ASSERT(ptr);
...
void AcpiX86::ParseTableHPET(AcpiHeaderHPET* header) {
  RT_ASSERT(header);
  if (nullptr != hpet_) {
...
void AcpiX86::ParseTableAPIC(AcpiHeaderMADT* header) {
  RT_ASSERT(header);
  ...
  while (p < end) {
    ...
    case ApicType::IO_APIC: {
      ...
      io_apics_.push_back(new IoApicX86(s->ioApicId,
...
void AcpiX86::StartCPUs() {
  CpuTrampolineX86 trampoline;
...
void AcpiX86::InitIoApics() {
  for (IoApicX86* ioa : io_apics_) {
    ioa->Init();
  }
}
