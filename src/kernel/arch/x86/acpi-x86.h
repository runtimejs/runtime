...
#include <string>
#include <vector>
#include <kernel/arch/x86/cpu-trampoline-x86.h>
#include <kernel/arch/x86/ioapic-x86.h>
#include <kernel/arch/x86/hpet-x86.h>
...
public:
  ...
  LocalApicX86* local_apic() const {
    return local_apic_;
  }
  void* local_apic_address() const {
    return local_apic_address_;
  }
  uint32_t cpus_count() const {
    return cpu_count_;
  }
...
private:
  LocalApicX86* local_apic_;
  void* local_apic_address_;
  HpetX86* hpet_;
  std::vector<AcpiCPU> cpus_;
  std::vector<IoApicX86*> io_apics_;
  uint32_t cpu_count_;
  ...
  DELETE_COPY_AND_ASSIGN(AcpiX86);
};
